...
function signAccessToken(user) {
  requireJwtSecret();
  return jwt.sign({ sub: user.id, email: user.email, plan: user.plan }, JWT_SECRET, { expiresIn: ACCESS_TTL, issuer: 'dj-ai' });
}

function verifyAccessToken(token) {
  requireJwtSecret();
  return jwt.verify(token, JWT_SECRET, { issuer: 'dj-ai' });
}
...
async function createUser({ email, password, displayName }) {
  ...
  const result = await query(
    'INSERT INTO users(email,password_hash,display_name) VALUES($1,$2,$3) RETURNING id,email,display_name,plan,email_verified_at,created_at',
    [normalized, passwordHash, String(displayName || '').trim().slice(0, 120)]
  );
  const user = result.rows[0];
  await query('INSERT INTO workspaces(owner_user_id,name) VALUES($1,$2) RETURNING id', [user.id, 'Personal']);
  return user;
}
